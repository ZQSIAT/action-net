...
class I3D(nn.Module):
    ...
    def __init__(self, num_classes=16, dropout_drop_prob=0.5, in_channel=3, duration=8, spatial_squeeze=True):
        super(I3D, self).__init__()
        last_duration = int(math.ceil(duration / 8))
        ...
        self.features = nn.Sequential(
            ...
            Mixed_5b(), # (832, 8, 7, 7)
            Mixed_5c(), # (1024, 8, 7, 7)
            nn.AvgPool3d(kernel_size=(last_duration, 7, 7), stride=1),# (1024, 8, 1, 1)
...
if __name__ == '__main__':
    ...
    model = I3D(num_classes=16, in_channel=3)
    out = model(input)
    print(out.shape)
