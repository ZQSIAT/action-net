...
def constructing_model(args, config, logger):
    ...
    # print(arch_import_str)
    # raise RuntimeError
    # todo
    exec(arch_import_str)

    # For skeleton data the "args.data_shape" in format of [FxCxHxW] without batch_zise
    net_arch_kwargs = "(num_classes={:d}, in_channel={:d}, num_segments={:d})".format(num_classes,
                                                                                      args.data_shape[1],
                                                                                      args.data_shape[0])
    # print(net_arch_kwargs)
    # exit()

    # # args.data_shape in format of [CxFxHxW] without batch_zise
    # net_arch_kwargs = "(num_classes={:d}, in_channel={:d}, num_segments={:d})".format(num_classes, args.data_shape[0], args.data_shape[1])
