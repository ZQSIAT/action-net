...
class NTURGBD_SKELETON(data.Dataset):
    def __init__(self, index_parm, spatial_transform=None, temporal_transform=None):
        ...
        self.combinations_3_15 = create_combination(3, 15)
        pass
    pass
    ...
    def _loading_data(self, file_path, file_count):
        if self.modality == "Skeleton":
            ...
            if self.is_skeleton_transform_velocity:
                ...
                data_processed = read_ntu_skeleton3p_cos_3x16x455x1(file_path, file_count, skeleton_loader=mat_load, file_idx=file_idx, combinations_3_20=self.combinations_3_15)
                # data_processed = read_ntu_skeleton3p_cos_3x16x1140x1(file_path, file_count, skeleton_loader=mat_load, file_idx=file_idx, combinations_3_20=self.combinations_3_15)
...
def read_ntu_skeleton3p_cos_3x16x1140x1(skeleton_file_dir, skeleton_count, skeleton_loader=None, file_idx=None, combinations_3_20=None):
    ...
    # dst_array = (dst_array - 0.831969) / 1.475146  # utd_mhad
    # dst_array = (dst_array - 0.707839) / 1.156303  # utd_mvhad
    # dst_array = (dst_array - 988.210054) / 1812.373133  # UWA3D

    # print(dst_array[:,[0,1,2]]-dst_array[:,[3,4,5]])
    # print((dst_array[:,[0,1,2]]-dst_array[:,[3,4,5]]).shape)
    # raise RuntimeError
    copy_dst_array = np.tile(dst_array, (len(combinations_3_20), 1))
    copy_dst_array = copy_dst_array.reshape([len(combinations_3_20), len(file_idx), 60])  # 455*16*45
    # print(np.array(copy_dst_array[2,:,[0,1,2]]-copy_dst_array[0,:,[3,4,5]]).T)
    # print(np.array(copy_dst_array[2,:,[0,1,2]]-copy_dst_array[0,:,[3,4,5]]).T.shape)
    # raise RuntimeError
    for i,ICount in enumerate(combinations_3_20):
    ...
    pass
def read_ntu_skeleton3p_cos_3x16x455x1(skeleton_file_dir, skeleton_count, skeleton_loader=None, file_idx=None, combinations_3_20=None):
    ...
    # raise RuntimeError
    copy_dst_array = np.tile(dst_array, (len(combinations_3_20), 1))
    copy_dst_array = copy_dst_array.reshape([len(combinations_3_20), len(file_idx), 45])  # 455*16*45
    # print(np.array(copy_dst_array[2,:,[0,1,2]]-copy_dst_array[0,:,[3,4,5]]).T)
    # print(np.array(copy_dst_array[2,:,[0,1,2]]-copy_dst_array[0,:,[3,4,5]]).T.shape)
    # raise RuntimeError
    for i,ICount in enumerate(combinations_3_20):
