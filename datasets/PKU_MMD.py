# -*- coding: utf-8 -*-
r"""This python file is designed to read the PKU-MMD dataset
"""
# import cv2
import os
import math
...
class PKUMMD(data.Dataset):
    ...
    def _loading_data(self, file_path, file_count):
        ...
        if self.modality == "Depth":
            ...
            if self.is_depth_clip:
                data_processed = read_pku_depth_maps_pil(file_path,
                                                         self.depth_format,
                ...
                data_processed = read_pku_depth_maps(file_path,
                                                     self.depth_format,
...
def read_pku_depth_maps(depth_file_dir,
                        depth_format,
                        img_loader=None,
                        file_idx=None,
                        norm_val=False,
    ...
    if file_idx is not None:
        depth_file_list = [depth_format.format(idx=idx) for idx in file_idx]
    else:
...
def read_pku_depth_maps_pil(depth_file_dir,
                            depth_format,
                            img_loader=None,
                            file_idx=None,
                            norm_val=False,
...
if __name__ == '__main__':
    ...
    index_param["is_vector"] = False
    index_param["file_format"] = "/ssd_data/zqs/datasets/pku_mmd/depth"
    index_param["file_subset_list"] = "/home/zqs/workspace-xp/deployment/action-net-47/PKU_MMD_output/protocols/PKU_MMD_1_cross_subjects_train.txt"
    ...
    modality = "Depth"
    spatial_method = "spatial_group_crop_org_scale"
    temporal_method = "dynamic_snapshot_sampling"

    loader_param = config.get_loader_param("PKU_MMD", modality)
    ...
    data_loader = PKUMMD(index_param, spatial_transform=spatial_transform,
                                temporal_transform=temporal_transform)

    data_len = data_loader.__len__()

    depth_maps, label, _ = data_loader.__getitem__(1)
